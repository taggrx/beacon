...
impl State {
    ...
    pub fn create_order(
        ...
        if order_type == "buy" {
            ...
            if volume + max_fee > self.icp_pool.get(&caller).copied().unwrap_or_default() {
    ...
    pub fn trade(
        ...
        let icp_pool_tokens: E8s = self.icp_pool.values().sum();
        ...
        while let Some(mut order) = if buying {
            ...
            amount = if order.amount > amount {
                ...
                if buying
                    && volume + trading_fee(volume) > *self.icp_pool.get(&trader).unwrap_or(&0)
                {
                    orders.insert(order);
                    break;
                }
        ...
        if icp_pool_tokens != self.icp_pool.values().sum::<E8s>() {
...
mod tests {
    ...
    fn test_liquidity_adding_and_withdrawals() {
        ...
        assert_eq!(
            ...
            Ok((ic_ledger_types::Tokens::from_e8s(one_icp) - icp::fee()).e8s())
        );
    }
    ...
    fn test_selling() {
        ...
        assert_eq!(state.icp_pool.get(&pr(2)).unwrap(), &(26 * 1000000));
        ...
        // three buyers
        assert_eq!(state.icp_pool.len(), 3);
        ...
        assert_eq!(state.icp_pool.get(&seller).unwrap(), &(v1 + v2 + v3 - fees));
        assert_eq!(state.icp_pool.get(&pr(255)).unwrap(), &(2 * fees));
    }
    ...
    fn test_buying() {
        ...
        state
            ...
            .unwrap();
        assert_eq!(state.icp_pool.len(), 1);
        assert_eq!(state.trade("buy", buyer, token, 10, None, 123456), Ok(()));
        ...
        assert_eq!(state.icp_pool.get(&pr(0)).unwrap(), &(v1 - trading_fee(v1)));
        assert_eq!(state.icp_pool.get(&pr(1)).unwrap(), &(v2 - trading_fee(v2)));
        assert_eq!(state.icp_pool.get(&pr(2)).unwrap(), &(v3 - trading_fee(v3)));
    ...
    fn test_limit_buying() {
        ...
        assert_eq!(state.icp_pool.get(&pr(0)).unwrap(), &(v1 - trading_fee(v1)));
        assert_eq!(state.icp_pool.get(&pr(1)).unwrap(), &(v2 - trading_fee(v2)));
        assert_eq!(state.icp_pool.get(&pr(2)), None);
    }
}
